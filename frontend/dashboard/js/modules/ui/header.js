...
// Управление шапкой: темы фона, фильтры статусов и контекстное состояние UI
...
export const updateHeaderContext = (tab) => {
    ...
    document.querySelectorAll('#toggleView, .stat-box.clickable').forEach(el => {
        Object.assign(el.style, { opacity: isBots ? '1' : '0.3', pointerEvents: isBots ? 'auto' : 'none' });
        el.classList.toggle('disabled', !isBots);
        !isBots && el.classList.remove('active');
    });
};
...
export const setActiveFilterUI = (filter, isGrid) => {
    document.querySelectorAll('.stat-box.clickable').forEach(el => {
        ...
        Object.assign(el.style, { opacity: disabled ? '0.3' : '1', pointerEvents: disabled ? 'none' : 'auto' });
    });
};

export const initializeHeader = (callbacks) => {
    const modal = document.getElementById('bgModal');
    const grid = modal?.querySelector('.bg-options-grid');
    
    grid && (grid.innerHTML = BG_LIST.map(n => `
    ...
    document.addEventListener('click', (e) => {
        ...
        btn.classList.contains('stat-box') && callbacks.onFilterChange(btn.id.replace('filter-', ''));
    });
};