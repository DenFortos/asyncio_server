...
export const setClientPreview = (id, url) => {
    // Создаем запись, если превью пришло раньше SystemInfo
    if (!clients[id]) clients[id] = { id, status: 'offline' };
    
    // Чистим память от старого Blob
    if (clients[id].lastPreview?.startsWith('blob:')) {
        URL.revokeObjectURL(clients[id].lastPreview);
    }
    
    clients[id].lastPreview = url;
    emit(); // Триггер для Renderer.js
};
