...
export function encodePacket(id, mod, pay = "") {
    ...
    v.setUint8(0, bId.length); v.setUint8(1, bMod.length); v.setUint32(2, bPay.length, false);
    res.set(bId, 6); res.set(bMod, 6 + bId.length); res.set(bPay, 6 + bId.length + bMod.length);
    return res.buffer;
}