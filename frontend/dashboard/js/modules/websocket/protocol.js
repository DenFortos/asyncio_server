...
// Декодирование бинарного пакета: извлечение ID, названия модуля и полезной нагрузки
export const decodePacket = (buf) => {
    ...
    try {
        ...
        return {
            id: dec.decode(new Uint8Array(buf, 6, idL)).replace(/\0/g, ''),
            module: dec.decode(new Uint8Array(buf, 6 + idL, modL)),
            payload: buf.slice(6 + idL + modL, 6 + idL + modL + payL)
        };
    } catch (e) { return null; }
};

// Кодирование данных в бинарный пакет согласно структуре протокола
